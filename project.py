...
def api_connect():
    ...
    api_version = "v3"
    api_key="AIzaSyCtyCLC4x61pIc_ah1drElH9XlKEIHVhGA"
...
com_info = get_comment_data(videoid_info)
...
mycursor.execute('use youtubeproject')
...
if channel_id:
    ...
    Insert_Query = '''INSERT INTO channel_data(channel_id,channel_name,channel_sub,channel_view,channel_desc,channel_playlistid,channel_status,channel_video) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)'''
        ...
        # Check if the channel_id already exists in the table
            ...
            if not existing_channel:
                val = (
                    ...
                    channel_infos['channel_sub'],
                    channel_infos['channel_view'],
                    channel_infos['channel_desc'],
...
try:
    for video_data in video_info:
        if not video_exists(video_data['Video_ID']):
            sql = """INSERT INTO videos_data(channel_name, Video_ID, Video_Title, Thumbnail, Description, PublishedAt,
            ...
            val = (video_data['channel_name'], video_data['Video_ID'], video_data['Video_Title'],
                    video_data['Thumbnail'], video_data['Description'], PublishedAt, duration_formatted,
...
def fetch_channel_db():
    ...
    df1 = pd.DataFrame(data, columns=['channel_id','channel_name','channel_sub','channel_view','channel_desc',
...
def fetch_video_db():
    ...
    data = mycursor.fetchall()
    df3 = pd.DataFrame(data, columns=['channel_name','Video_ID','Video_Title','Thumbnail','Description', 
...
def main():
    ...
    #----5----
        ...
        query5 = '''SELECT channel_name, Video_Title, Like_Count FROM videos_data WHERE Like_Count IS NOT NULL ORDER BY Like_Count DESC'''
    ...
    #----6----
        ...
        query6 = '''SELECT Video_Title, Like_Count, Dislike_Count FROM videos_data'''
    ...
    #----7----
        ...
        query7 = '''SELECT channel_name, SUM(Video_views) AS Total_Views FROM videos_data WHERE Video_views IS NOT NULL GROUP BY channel_name'''
    ...
    #----8----
        ...
        query8 = '''SELECT DISTINCT channel_name FROM videos_data WHERE Publish_Date LIKE '%2022%' '''
    ...
    #----9----
        ...
        query9 = '''SELECT channel_name, Video_Title, Video_comment FROM videos_data WHERE Video_comment IS NOT NULL ORDER BY Video_comment DESC'''
    ...
    #----10----
    elif question == "10. What is the average duration of all videos in each channel, and what are their corresponding channel names?":
        query10 = '''SELECT channel_name, AVG(Duration) AS Avg_Duration FROM videos_data WHERE Duration IS NOT NULL GROUP BY channel_name'''
